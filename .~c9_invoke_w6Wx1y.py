...
def buy():
    ...
    if request.method == "POST":
        ...
        if not stock_data:
            return apology ("Check symbol", 420)

        rows = db.execute("SELECT * FROM users WHERE id = :user_id", user_id=session["user_id"])
        stock_cost = float(stock_data["price"]) * float(request.form.get("share_amount"))

        if rows[0]["cash"] < stock_cost:
            return apology ("Ya pleb", 420)
            ...
            db.execute("INSERT INTO transactions (user_id, symbol, bought, amount, shares)"
                        ...
                        shares=int(request.form.get("share_amount")))
            ...
            result = db.execute("UPDATE OR ABORT owned SET shares = shares + :new_shares"
                                ...
                                new_shares=int(request.form.get("share_amount")))
            ...
            if not result:
                db.execute("INSERT INTO owned (user_id, symbol, shares) VALUES (:user_id, :symbol, :shares)",
                            user_id=session["user_id"], symbol=stock_data["symbol"],
                            shares=request.form.get("share_amount"))
...
def quote():
    ...
    if request.method == "POST":
        ...
        stock_data = lookup(request.form.get("symbol"))
        if not stock_data:
            return apology ("Try again loser", 421)
...
def sell():
    ...
    if request.method == "GET":
        ...
        if not rows:
            return apology ("You own nothing", 420)
